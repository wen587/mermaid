...
sequenceDiagram
    ...
    client->>gnmi splitter: read DPU_CAPACITY table for DPU0
    gnmi splitter->>gnmi server for DPU0: read DPU_CAPACITY table
    gnmi server for DPU0->>DPU_STATE_DB for DPU0: read DPU_CAPACITY table
    DPU_STATE_DB for DPU0-->>gnmi server for DPU0: result
    gnmi server for DPU0-->>gnmi splitter: result
    gnmi splitter-->>client: result
    client->>gnmi splitter: read DPU_CAPACITY table for DPU1
    gnmi splitter->>gnmi server for DPU1: read DPU_CAPACITY table
    gnmi server for DPU1->>DPU_STATE_DB for DPU1: read DPU_CAPACITY table
